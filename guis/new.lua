			...
			if categorysettings.Profiles then
				...
				if ind then
					if val ~= 'default' then
						...
						if isfile('newvape/profiles/'..val..game.GameId..'.txt') and delfile then
							delfile('newvape/profiles/'..val..game.GameId..'.txt')
						end
					end
				else
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('newvape/profiles/'..self.Profile..game.GameId..'.txt') then
		local savedata = loadJson('newvape/profiles/'..self.Profile..game.GameId..'.txt')
...
function mainapi:Save(newprofile)
	...
	writefile('newvape/profiles/'..self.Profile..game.GameId..'.txt', httpService:JSONEncode(savedata))
end
	...
	for i, profileName in ipairs(premades) do
		...
		loadButton.MouseButton1Click:Connect(function()
			...
			if isfile(premadeFile) then
				...
				if isfile('newvape/profiles/'..profileName..game.GameId..'.txt') and delfile then
					pcall(function() delfile('newvape/profiles/'..profileName..game.GameId..'.txt') end)
				end
				
				local premadeData = readfile(premadeFile)
				
				writefile('newvape/profiles/'..newProfileName..game.GameId..'.txt', premadeData)
...
general:CreateButton({
	...
	mainapi.Save = function() end
		if isfile('newvape/profiles/'..mainapi.Profile..game.GameId..'.txt') and delfile then
			delfile('newvape/profiles/'..mainapi.Profile..game.GameId..'.txt')
		end
